...
export async function GET(request: NextRequest) {
    ...
    const goodsImages = await prisma.goodsImage.findMany({
        skip,
        take: limit
    })

    const total = await prisma.goodsImage.aggregate({
        _count: true
    })

    let response = {
        status: 'success',
        results: goodsImages.length,
        pagination: {
            page: pageNumber,
            perPage: limit
        },
...
export async function POST(request: Request) {
   try {
        ...
        const goodsImage = await prisma.goodsImage.create({
            data: requestData
        })
